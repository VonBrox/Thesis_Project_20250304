...
### Then filter to n% most up/downregulated genes based on log2FC
## A1
...
## Now try the hypergeometric test
...
# Universe of genes (e.g., all genes in the study - 
# So if we are comparing genes overlapping after the percentile filtering step, we should be using the list from right before that: symbols shared between A1 and TE with FDR < 0.05 in both, therefore with chance of overlap)
...
# Hey nice, if we look at it this way, it is actually indeed significant
```
