...
hist(as.numeric(LUAD_A1_TE_ranked_genes$rank_order), main = "LUAD_A1_TE genes", xlab = "Rank", ylim = c(0,80))
...
### Apply getMeanSD function to thegene list to generate a table of SMD values
...
LUAD_A1_TE_SMD_summary_overall$FDR_random <- p.adjust(as.numeric(LUAD_A1_TE_SMD_summary_overall$p_value_random), method = "BH")

# Save summary
write.table(LUAD_A1_TE_SMD_summary_overall, "../../2_Outputs/6_Meta_analysis/LUAD_A1_TE_SMD_summary_overall_20250611.txt")
...
## ENRICHR link:
## https://maayanlab.cloud/Enrichr/enrich?dataset=a1a24eb2bd3d8e61489e14732d4966c6

# Trying out just filtering FDR
LUAD_A1_TE_SMD_summary_overall_filtered <- LUAD_A1_TE_SMD_summary_overall %>%
  filter(FDR_random <= 0.05)
#ENRICHR link: https://maayanlab.cloud/Enrichr/enrich?dataset=37b93ca2a5d4a7436d8c19e4ae7a24ae
...
df_SMD_forest <- forest(df_SMD_values,
                        ...
                        smlab = paste0(gene, " SMD forest plot"),
  ...
  for (j in 1:length(df_list_goi)){
    ...
    boxplot<-ggplot(combined_data_df, aes(x = Group, y = Expression, fill = Group)) +
      ...
      labs(title = paste0(curr_gene, ": Current vs Never Smokers expression"),
             # x = "Group", 
             y = "Expression (RMA probe intensity)") +
