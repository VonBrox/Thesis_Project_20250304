...
2025/05/14: I am now using the genes that were filtered using my new "sequential filtering method" in the order A1-TE and A1-TE-A2, for LUSC
...
### 2025/05/14: For now I will use LUSC-A1-TE
```{r}
library(meta)

## Use the persistent linked genes list as the input genes list
gene_list <- LUSC_A1_TE
...
LUSC_A1_TE_SMD_summary <- data.frame(
  GEO_ID = character(0),
  gene = character(0),
  ID = character(0),
  n_obs = numeric(0),
...
for (i in 1:nrow(gene_list)){ # For each gene on the gene list
  ...
  # Append results from this to the output dataframe
    LUSC_A1_TE_SMD_summary <- rbind(LUSC_A1_TE_SMD_summary, df_SMD_summary)
  }
}

head(LUSC_A1_TE_SMD_summary)
```
...
LUSC_A1_TE_SMD_summary_ID_filter <- LUSC_A1_TE_SMD_summary %>%
...
## Use LUSC_A1_TE as the input genes list
gene_list <- LUSC_A1_TE
...
# Make a dataframe that will have all the SMD output values
LUSC_A1_TE_SMD_summary_overall <- data.frame(
  GEO_ID = character(0),
  gene = character(0),
  n_obs = numeric(0),
  SMD = numeric(0),
...
for (i in 1:nrow(gene_list)){ # For each gene on the gene list
  ...
  # Append results from this to the output dataframe
  LUSC_A1_TE_SMD_summary_overall <- rbind(LUSC_A1_TE_SMD_summary_overall, df_SMD_summary_overall)
  
}

head(LUSC_A1_TE_SMD_summary_overall)

## Save the summary
write.table(LUSC_A1_TE_SMD_summary_overall, "../2_Outputs/LUSC_A1_TE_SMD_summary_overall_20250516.txt")
...
## For now it is too much to plot all the significant genes from LUSC-A1-TE, so I will try to plot the genes from the smaller list LUSC-A1-TE-A2

gene_list <- LUSC_A1_TE_A2
...
2025/05/16:For former smoker LUSC boxplots, I will check all of the genes that came out as SMD significant for LUSC-A1-TE-A2 so that we can see the difference more clearly in a visual way across the datasets.
...
# Make a list of the genes of interest (LUSC-A1-TE-A2 and significant SMD) for the boxplots
genes_of_interest <- c("AKR1B10", "CA12", "EFEMP1", "LMO2", "MAOB", "MUC5B", "UPK1B")
...
current_smoker_indexes <- list(GGSE4302_csi = GSE4302_current_smoker_indexes, GSE7895_csi = GSE7895_current_smoker_indexes, GSE19027_csi = GSE19027_current_smoker_indexes, GSE20257_csi = GSE20257_current_smoker_indexes, GSE63127_csi = GSE63127_current_smoker_indexes)
