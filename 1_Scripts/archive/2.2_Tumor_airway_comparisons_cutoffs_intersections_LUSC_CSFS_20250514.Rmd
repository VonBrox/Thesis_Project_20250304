...
print(paste0("overlap: ", nrow(A1_TE_merged_DEGs)))
...
## Hypergeometric test for LUSC-A1-TE genes
```{r}

N = length(intersect(GSE63127_CS_NS$Gene, TCGA_LUSC_DEG$Gene)) # Number of genes in both A1 and TE BEFORE the cutoffs are applied
N

n = nrow(GSE63127_CS_NS_cutoff) # Number of genes in LUSC-A1 meeting cutoff
n

m = nrow(TCGA_LUSC_DEG_cutoff) # Number of genes in LUSC-TE meeting cutoff
m

k  = nrow(A1_TE_merged_DEGs) # Number of overlapping genes for LUSC-A1-TE
k
...
GSE63127_CS_NS_A1_TE_shared_genes <- GSE63127_CS_NS_formatted %>%
  dplyr::filter(Gene %in% intersect(TCGA_LUSC_DEG_formatted$Gene, GSE63127_CS_NS_formatted$Gene))
...
# Note: The genes that end up being duplicates for TCGA_LUSC_DEG_A1_TE_shared_genes here are "DNAJC9-AS1", "POLR2J4", "RGS5", hence the difference in nrow for the two above, however I think these get lost in later steps so it does not really matter here.
...
## TE
# Calculate percentile cutoffs
top_cutoff_TE <- quantile(TCGA_LUSC_DEG_A1_TE_sig_all$log2FC_TE, 0.875, na.rm = TRUE)  # Top 12.5%
bottom_cutoff_TE <- quantile(TCGA_LUSC_DEG_A1_TE_sig_all$log2FC_TE, 0.125, na.rm = TRUE)  # Bottom 12.5%
...
m = nrow(TCGA_LUSC_DEG_A1_TE_sig)  # Number of genes with FDR <= 0.05 in TE (filter step 3)
print(paste0("m = ",m))
...
N = nrow(TCGA_LUSC_DEG_A1_TE_sig_all) # The number of genes with FDR <= 0.05 in both A1 and TE (after filter step 3)
print(paste0("N = ",N))

n = nrow(GSE63127_CS_NS_percentile) # Number of genes for top 75th percentile |log2FC| in each (here A1)
print(paste0("n = ",n))

m = nrow(TCGA_LUSC_DEG_percentile)  # Number of genes for top 75th percentile |log2FC| in each (here TE)
print(paste0("m = ",m))
