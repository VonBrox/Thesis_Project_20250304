...
GSE63127_CS_NS_A1_TE_shared_genes <- GSE63127_CS_NS_formatted %>%
  dplyr::filter(Gene %in% intersect(TCGA_LUAD_DEG_formatted$Gene, GSE63127_CS_NS_formatted$Gene))
...
# Note: The genes that end up being duplicates for TCGA_LUAD_DEG_A1_TE_shared_genes here are "DNAJC9-AS1", "POLR2J4", "RGS5", hence the difference in nrow for the two above, however I think these get lost in later steps so it does not really matter here.
...
A1_TE_merged_DEGs_percentile <- GSE63127_CS_NS_percentile %>%
  inner_join(., TCGA_LUAD_DEG_percentile, by = "Gene")
nrow(A1_TE_merged_DEGs_percentile)

print("Filter 6: Same sign for log2FC")
A1_TE_merged_DEGs_percentile_sign <- A1_TE_merged_DEGs_percentile %>%
  filter(sign(log2FC_A1)==sign(log2FC_TE)) # Filter to genes with same signs
nrow(A1_TE_merged_DEGs_percentile_sign)

print("Merge with A2, same sign")
A1_TE_A2_linked_genes_method2 <- A1_TE_merged_DEGs_percentile_sign %>%
  inner_join(., GSE7895_persistent_DEGs, by = "Gene") %>%
  filter(sign(log2FC_A1)==sign(CS_NS_A2)) # Filter to genes with same sign
nrow(A1_TE_A2_linked_genes_method2)
...
N = nrow(GSE63127_CS_NS_A1_TE_shared_genes) # All gene symbols shared between A1 and TE (filter step 1)
print(paste0("N = ",N))

n = nrow(GSE63127_CS_NS_A1_TE_sig) # Number of genes with FDR <= 0.05 in A1 (filter step 3)
print(paste0("n = ",n))

m = nrow(TCGA_LUAD_DEG_A1_TE_sig)  # Number of genes with FDR <= 0.05 in TE (filter step 3)
print(paste0("m = ",m))

k  = nrow(GSE63127_A1_TE_sig_all) # Number of genes with FDR <= 0.05 in both A1 and TE
print(paste0("k = ",k))
...
N = nrow(TCGA_LUAD_DEG_A1_TE_sig_all) # The number of genes with FDR <= 0.05 in both A1 and TE (after filter step 3)
print(paste0("N = ",N))

n = nrow(GSE63127_CS_NS_percentile) # Number of genes for top 75th percentile |log2FC| in each (here A1)
print(paste0("n = ",n))

m = nrow(TCGA_LUAD_DEG_percentile)  # Number of genes for top 75th percentile |log2FC| in each (here TE)
print(paste0("m = ",m))

k  = nrow(A1_TE_merged_DEGs_percentile_sign) # Number of genes with top 75th percentile in both and with same sign
print(paste0("k = ",k))
...
for (row in 1:1000) {
 ...
 #   filter(abs(log2FC)>log2FC_cutoff_TM) %>% ###NOTE: Not sure I should be doing this filter here!!! Might change the result!!
  #  filter(FDR < FDR_cutoff_TM) %>%
    dplyr::rename(., FDR_TM = FDR, log2FC_TM = log2FC) %>%
    dplyr::select(., Gene, FDR_TM, log2FC_TM)
...
union_size <- length(unique(c(GSE63127_CS_NS_cutoff$Gene, TCGA_LUAD_DEG_cutoff$Gene, TCGA_LUAD_DMG_cutoff$Gene))) # Size of |A ∪ B ∪ C|
...
# Print expected and observed overlaps
## Expected/observed overlap given independent datasets:
expected_overlap <- nrow(GSE63127_CS_NS_cutoff)/20000*nrow(TCGA_LUAD_DEG_cutoff)/20000*nrow(TCGA_LUAD_DMG_cutoff)
...
TE_genes <- TCGA_LUAD_DEG_cutoff$Gene
A2_genes <- GSE7895_persistent_DEGs$Gene
TM_genes <- TCGA_LUAD_DMG_cutoff$Gene
...
###  Visualizing Spearman correlations of log2FC values
```{r}
...
### Visualizing multiple linear regression of the log2FC values
```{r}
...
TCGA_LUAD_DEG_sig <- TCGA_LUAD_DEG_shared_genes %>%
  filter(FDR_TE <= 0.05)
nrow(TCGA_LUAD_DEG_sig)

TCGA_LUAD_DMG_sig <- TCGA_LUAD_DMG_shared_genes %>%
  filter(FDR_TM <= 0.05)
nrow(TCGA_LUAD_DMG_sig)


# Then filter to FDRs being <= 0.05 in all 3
GSE63127_CS_NS_sig_all <- GSE63127_CS_NS_sig %>%
  dplyr::filter(Gene %in% TCGA_LUAD_DEG_sig$Gene) %>%
  dplyr::filter(Gene %in% TCGA_LUAD_DMG_sig$Gene)
nrow(GSE63127_CS_NS_sig_all)

TCGA_LUAD_DEG_sig_all <- TCGA_LUAD_DEG_sig %>%
  dplyr::filter(Gene %in% TCGA_LUAD_DMG_sig$Gene) %>%
  dplyr::filter(Gene %in% GSE63127_CS_NS_sig$Gene)
nrow(TCGA_LUAD_DEG_sig_all)

TCGA_LUAD_DMG_sig_all <- TCGA_LUAD_DMG_sig %>%
  dplyr::filter(Gene %in% GSE63127_CS_NS_sig$Gene) %>%
  dplyr::filter(Gene %in% TCGA_LUAD_DEG_sig$Gene)
nrow(TCGA_LUAD_DMG_sig_all)
...
A1_TE_merged_DEGs <- GSE63127_CS_NS_sig_all %>%
  inner_join(., TCGA_LUAD_DEG_sig_all, by = "Gene") %>%
  filter(sign(log2FC_A1)==sign(log2FC_TE)) # Filter to genes with same signs
nrow(A1_TE_merged_DEGs)
...
TCGA_LUAD_DEG_ngenes <- nrow(TCGA_LUAD_DEG_sig)
GSE7895_persistent_DEGs_ngenes <- nrow(GSE7895_persistent_DEGs) # no cutoff was applied here
TCGA_LUAD_DMG_ngenes <- nrow(TCGA_LUAD_DMG_sig)
...
for (row in 1:1000) {
  ...
  TCGA_LUAD_DEG_sample <- TCGA_LUAD_DEG_shared_genes %>% 
    sample_n(TCGA_LUAD_DEG_ngenes) 
    
  GSE7895_persistent_DEGs_sample <- GSE7895_persistent_DEGs %>% 
    sample_n(GSE7895_persistent_DEGs_ngenes) #here it changes nothing because we are just sampling everything
  
  TCGA_LUAD_DMG_sample <- TCGA_LUAD_DMG_shared_genes %>% 
    sample_n(TCGA_LUAD_DMG_ngenes)
  
  # Check the number of shared gene symbols between A1, TE, TM overall
  GSE63127_CS_NS_sample_shared <- GSE63127_CS_NS_sample %>%
    filter(Gene %in% TCGA_LUAD_DEG_sample$Gene) %>%
    filter(Gene %in% TCGA_LUAD_DMG_sample$Gene)
  
  TCGA_LUAD_DEG_sample_shared <- TCGA_LUAD_DEG_sample %>%
    filter(Gene %in% GSE63127_CS_NS_sample$Gene) %>%
    filter(Gene %in% TCGA_LUAD_DMG_sample$Gene)
  
  TCGA_LUAD_DMG_sample_shared <- TCGA_LUAD_DMG_sample %>%
    filter(Gene %in% GSE63127_CS_NS_sample$Gene) %>%
    filter(Gene %in% TCGA_LUAD_DEG_sample$Gene)
  ...
  A1_TE_merged_DEGs_sample <- GSE63127_CS_NS_sample_shared %>%
    inner_join(., TCGA_LUAD_DEG_sample_shared, by = "Gene") %>%
    filter(sign(log2FC_A1)==sign(log2FC_TE)) # Filter to genes with same signs
  ...
  # Check the random overlaps with TM with the sign filter
  A1_TE_A2_TM_genes_sample <- A1_TE_A2_genes_sample %>%
    dplyr::inner_join(., TCGA_LUAD_DMG_sample_shared, by = "Gene") %>%
    dplyr::filter(sign(log2FC_A1) != sign(log2FC_TM)) # Ensure signs are opposite
...
So actually this is NOT encouraging, really. What I got is not much different than chance for the first 2 overlaps/filters. The higher numbers for the next 2 overlaps could be explained by the inherent similarities between the datasets compared after that. Now let's try hypergeometric test with this new definition of gene universe, whoop-de-do.

## Hypergeometric test again
```{r}

library(SuperExactTest)

# Define gene lists - these are the genes defined as significant in each dataset (not in ALL)
A1_genes <- GSE63127_CS_NS_sig$Gene
TE_genes <- TCGA_LUAD_DEG_sig$Gene
A2_genes <- GSE7895_persistent_DEGs$Gene
TM_genes <- TCGA_LUAD_DMG_sig$Gene

# Universe of genes (e.g., all genes in the study)
universe <- 12387 # Shared gene symbols between the datasets
...
# This is not good. Looks like it's all due to chance except for the datasets that are of COURSE related. Shitt.
...
# Filter to FDRs being <= 0.05 in just these 2 datasets
GSE63127_CS_NS_sig_A1_TE <- GSE63127_CS_NS_sig %>%
  dplyr::filter(Gene %in% TCGA_LUAD_DEG_sig$Gene)
nrow(GSE63127_CS_NS_sig_A1_TE)

TCGA_LUAD_DEG_sig_A1_TE <- TCGA_LUAD_DEG_sig %>%
  dplyr::filter(Gene %in% GSE63127_CS_NS_sig$Gene)
nrow(TCGA_LUAD_DEG_sig_A1_TE)

## Combine them together
A1_TE <- GSE63127_CS_NS_sig_A1_TE %>%
  left_join(., TCGA_LUAD_DEG_sig_A1_TE, by = "Gene", suffix = c("_A1", "_TE"))
...
cor_test
# Interesting, it is highly significant? How?
