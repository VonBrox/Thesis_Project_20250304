...
### Low read counts filtering methods ###

#  Method 1: Filter out rows with less than 20 total counts in the smallest sample group size (114/2 = 57)
keep <- rowSums(counts_matched_T_NM >= 10) >= 57

# Method 2: Use CPM with the same approach
#cpm_values <- cpm(counts_matched_T_NM)
keep <- rowSums(cpm_values >= 1) >= 57

# Method 3: Use the filterByExpr package 

keep <- filterByExpr(counts_matched_T_NM, design = design, min.count=20, min.total.count = 15, min.prop = 1)

# For testing
counts_matched_T_NM_checking <- counts_matched_T_NM[keep,]
