...
### Then filter to 50% most up/downregulated genes based on log2FC
## A1
...
GSE63127_CS_NS_50th_percentile <- GSE63127_A1_TE_sig_all %>%
  filter(log2FC_A1 >= top_cutoff_A1 | log2FC_A1 <= bottom_cutoff_A1)

nrow(GSE63127_CS_NS_50th_percentile)
...
# Filter genes in the top and bottom 25% of log2FC
TCGA_LUAD_DEG_50th_percentile <- TCGA_LUAD_DEG_A1_TE_sig_all %>%
  filter(log2FC_TE >= top_cutoff_TE | log2FC_TE <= bottom_cutoff_TE)

nrow(TCGA_LUAD_DEG_50th_percentile)
...
## Merge A1 and TE
A1_TE_merged_DEGs <- GSE63127_CS_NS_50th_percentile %>%
  inner_join(., TCGA_LUAD_DEG_50th_percentile, by = "Gene") %>%
  filter(sign(log2FC_A1)==sign(log2FC_TE)) # Filter to genes with same signs
nrow(A1_TE_merged_DEGs)
