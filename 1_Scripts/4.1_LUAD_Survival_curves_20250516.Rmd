...
## Save data for loading since it takes so long to download
saveRDS(data, file = "../../2_Outputs/LUAD_counts_data_20250522.rds")

# Load it later
rse_object <- readRDS("my_rse.rds")
...
genes_of_interest <- LUAD_A1_TE_A2$Gene
...
for (gene in genes_of_interest){
  ...
  if (p_value <= 0.05) {
    ...
    ggsp <- ggsurvplot(fit, data = former_smokers_high_low_goi, pval = TRUE, title = paste0("LUAD Former smokers ", gene_of_interest))
    print(ggsp)
    dev.off()
  }
}

## 2025/05/22: Now it seems to only be INPP4B
